...
//    int w_sum = w,                            h_sum = h;
    ...
    resized |= tex_obs_FG.resize(context, GLES3Impl.GL_RGBA, w_obs, h_obs, GLES3Impl.GL_RGBA, GLES3Impl.GL_UNSIGNED_BYTE, GLES3Impl.GL_NEAREST, 4, 1);
    resized |= tex_obs   .resize(context, GLES3Impl.GL_RGBA, w_obs, h_obs, GLES3Impl.GL_RGBA, GLES3Impl.GL_UNSIGNED_BYTE, GLES3Impl.GL_NEAREST, 4, 1);
    
    boolean HIGHP_FLOAT = true;
    int format  = GLES3Impl.GL_RED;
    int filter  = GLES3Impl.GL_LINEAR;
    int wrap    = GLES3Impl.GL_CLAMP_TO_EDGE;
    int iformat = HIGHP_FLOAT ? GLES3Impl.GL_R32F : GLES3Impl.GL_R16F;
    int type    = HIGHP_FLOAT ? GLES3Impl.GL_FLOAT : GLES3Impl.GL_HALF_FLOAT;
  ...
  public void resizeParticlesCount(int w, int h){
    boolean resized = tex_particle.resize(context, GLES3Impl.GL_RGBA32F, w, h, GLES3Impl.GL_RGBA, GLES3Impl.GL_FLOAT, GLES3Impl.GL_NEAREST, 4, 4);
    if(resized){
      reset();
    }
  }
  ...
  public void createCollisionFlowField(){
    ...
    context.begin();
    {
      ...
      GLES30.glColorMask(true, false, false, false);
      GLES30.glClearColor(0,0,0,0);
      GLES30.glClear(GLES3Impl.GL_COLOR_BUFFER_BIT);
