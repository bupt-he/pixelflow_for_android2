...
//    tex_shading.clear(0.0f);
  ...
  public boolean resize(int w, int h){
    ...
    for(int i = 0; i < pg_shading.length; i++){
      ...
      if(resized[0]){
        DwUtils.changeTextureFormat(pg_shading[i], GLES3Impl.GL_R32F, GLES3Impl.GL_RED, GLES3Impl.GL_FLOAT, GLES3Impl.GL_LINEAR, GLES3Impl.GL_CLAMP_TO_EDGE);
      }
    }
    
//    resized[0] |= tex_shading.resize(context,  GLES3Impl.GL_R32F, w, h, GLES3Impl.GL_RED, GLES3Impl.GL_FLOAT, GLES3Impl.GL_LINEAR, GLES3Impl.GL_CLAMP_TO_EDGE, 1, 4);
