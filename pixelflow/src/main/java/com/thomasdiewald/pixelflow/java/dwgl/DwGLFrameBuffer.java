...
//      max_bind = max_draw_buffers;
  ...
  public void bind(DwGLTexture3D[] tex, int[] layer){
    ...
    for(int i = 0; i < count; i++){
      ...
      bind_targets          [i] = tex[i].target;
      // TODO: 2023/8/6
//      GLES30.glFramebufferTexture3D(GLES30.GL_FRAMEBUFFER, bind_color_attachments[i], tex[i].target, tex[i].HANDLE[0], 0, layer[i]);
    }
...
//          // TODO: 2023/8/6
////          GLES32.glFramebufferTexture3D(GLES30.GL_FRAMEBUFFER, bind_color_attachments[i], bind_targets[i], 0, 0, 0);
////          GLES32.glFramebufferTexture3D(GLES30.GL_FRAMEBUFFER, bind_color_attachments[i], bind_targets[i], 0, 0, 0);
//        } else {
...
//  }
  ...
  public void setRenderBuffer(int HANDLE_rbo, boolean depth, boolean stencil){
    ...
    if(!is_active) if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
      unbind();
    }
  }
