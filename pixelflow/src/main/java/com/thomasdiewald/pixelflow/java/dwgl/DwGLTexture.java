...
public class DwGLTexture{
  ...
  public void release(){
    ...
    // release pbo
    if(hasPBO()){
      GLES30.glDeleteBuffers(1, HANDLE_pbo, 0);
      HANDLE_pbo[0] = 0;
    }
  ...
  public boolean isTexture2(){
    return   GLES30.glIsTexture(HANDLE[0]);
  }

  public boolean hasPBO(){
    return (HANDLE_pbo[0] != 0);
  }
      ...
      int num_channel, int byte_per_channel, Buffer data)
  {
    ...
    if(B_RESIZE){
//    GLES3Impl.glTexParameteri(target, GL2.GL_TEXTURE_BASE_LEVEL, 0);
//    GLES3Impl.glTexParameteri(target, GL2.GL_TEXTURE_MAX_LEVEL, 0);

      GLES30.glPixelStorei(GL2.GL_UNPACK_ALIGNMENT, 1);
      GLES30.glPixelStorei(GL2.GL_PACK_ALIGNMENT  , 1);
//    int[] val = new int[1];
//    GLES3Impl.glGetIntegerv(GL2.GL_UNPACK_ALIGNMENT, val, 0);
//    System.out.println("GL_UNPACK_ALIGNMENT "+val[0]);
//    GLES3Impl.glGetIntegerv(GL2.GL_PACK_ALIGNMENT, val, 0);
//    System.out.println("GL_PACK_ALIGNMENT "+val[0]);

      GLES30.glTexImage2D   (target, 0, internalFormat, w, h, 0, format, type, data);
//    GLES3Impl.glTexSubImage2D(target, 0, 0, 0, w, h, format, type, data);
    }

    if(B_FILTER){
      GLES30.glTexParameteri(target, GL2.GL_TEXTURE_MIN_FILTER, filter); // GL_NEAREST, GL_LINEAR
      GLES30.glTexParameteri(target, GL2.GL_TEXTURE_MAG_FILTER, filter);
    }

    if(B_WRAP){
      GLES30.glTexParameteri (target, GL2.GL_TEXTURE_WRAP_S, wrap);
      GLES30.glTexParameteri (target, GL2.GL_TEXTURE_WRAP_T, wrap);
      GLES30.glTexParameterfv(target, GL2.GL_TEXTURE_BORDER_COLOR, new float[]{0,0,0,0}, 0);
    }
  ...
  public void setParamWrap(int wrap, float[] border_color){
    ...
    GLES30.glTexParameteri (target, GL2.GL_TEXTURE_WRAP_S, wrap);
    GLES30.glTexParameteri (target, GL2.GL_TEXTURE_WRAP_T, wrap);
    GLES30.glTexParameterfv(target, GL2.GL_TEXTURE_BORDER_COLOR, border_color, 0);
    GLES30.glBindTexture   (target, 0);
  }
  ...
  public void setParamFilter(int filter){
    this.filter = filter;
    GLES30.glBindTexture  (target, HANDLE[0]);
    GLES30.glTexParameteri(target, GL2.GL_TEXTURE_MIN_FILTER, filter);
    GLES30.glTexParameteri(target, GL2.GL_TEXTURE_MAG_FILTER, filter);
    GLES30.glBindTexture  (target, 0);
  }
  ...
  public void setParamFilter(int minfilter, int magfilter){
    GLES30.glBindTexture  (target, HANDLE[0]);
    GLES30.glTexParameteri(target, GL2.GL_TEXTURE_MIN_FILTER, minfilter);
    GLES30.glTexParameteri(target, GL2.GL_TEXTURE_MAG_FILTER, magfilter);
    GLES30.glBindTexture  (target, 0);
  }
  ...
  public void setParamBorder(float[] border){
    GLES30.glBindTexture   (target, HANDLE[0]);
    GLES30.glTexParameterfv(target, GL2.GL_TEXTURE_BORDER_COLOR, border, 0);
    GLES30.glBindTexture   (target, 0);
  }
  @RequiresApi(api = Build.VERSION_CODES.N)
  public void setParamBorder(int[] border){
    GLES30.glBindTexture    (target, HANDLE[0]);
    GLES32.glTexParameterIiv(target, GL2.GL_TEXTURE_BORDER_COLOR, border, 0);
    GLES30.glBindTexture    (target, 0);
  }
  ...
  */
  ...
  public void glTexParameteriv(int param, int[] value){
    GLES30.glTexParameteriv (target, param, value, 0);
  }
  public void glTexParameterf(int param, float value){
    GLES30.glTexParameterf (target, param, value);
  }
  public void glTexParameterfv(int param, float[] value){
    GLES30.glTexParameterfv(target, param, value, 0);
  }
  ...
  public void swizzle(int[] i4_GL_TEXTURE_SWIZZLE_RGBA){
    // TODO: 2023/8/6 解开
//    glTexParameteriv(GLES32.GL_TEXTURE_SWIZZLE_RGBA, i4_GL_TEXTURE_SWIZZLE_RGBA, true);
//    glTexParameteriv(GLES32.GL_TEXTURE_SWIZZLE_RGBA, i4_GL_TEXTURE_SWIZZLE_RGBA, true);
  }
  ...
  public void generateMipMap(){
    GLES30.glBindTexture   (target, HANDLE[0]);
    GLES30.glTexParameteri (target, GL2.GL_TEXTURE_MIN_FILTER, GL2.GL_LINEAR_MIPMAP_LINEAR);
//    GLES3Impl.glTexParameteri (target, GL2.GL_GENERATE_MIPMAP, GL2.GL_TRUE);
  ...
  private DwGLTexture createTexSubImage(int x, int y, int w, int h){
    ...
    GLES30.glBindTexture(target, texsub.HANDLE[0]);
    GLES30.glCopyTexSubImage2D(target, 0, 0, 0, x, y, w, h);
    GLES30.glBindTexture(target, 0);
  ...
  public ByteBuffer getData_GL2ES3(int x, int y, int w, int h){
    ...
    context.beginDraw(this);
    if(HANDLE_pbo[0] == 0){
      GLES30.glGenBuffers(1, HANDLE_pbo, 0);
    }
    GLES30.glBindBuffer(GL2.GL_PIXEL_PACK_BUFFER, HANDLE_pbo[0]);
    GLES30.glBufferData(GL2.GL_PIXEL_PACK_BUFFER, buffer_size, null, GL2.GL_DYNAMIC_READ);
    GLES30.glReadPixels(x, y, w, h, format, type, 0);

    ByteBuffer bbuffer = (ByteBuffer) GLES30.glMapBufferRange(GL2.GL_PIXEL_PACK_BUFFER, 0, buffer_size, GL2.GL_MAP_READ_BIT);
//    ByteBuffer bbuffer = GLES3Impl.glMapBuffer(GL2.GL_PIXEL_PACK_BUFFER, GL2.GL_READ_ONLY);

    GLES30.glUnmapBuffer(GL2.GL_PIXEL_PACK_BUFFER);
    GLES30.glBindBuffer(GL2.GL_PIXEL_PACK_BUFFER, 0);
