...
public class DwFlowField {
  ...
  public static class Param {
    ...
    public boolean HIGHP_FLOAT = false; // false=GLES3Impl.GL_RG16F, true=GLES3Impl.GL_RG32F; 
  }
  ...
  int tex_wrap = GLES3Impl.GL_CLAMP_TO_EDGE;
  ...
  public boolean resize(int w, int h){
    int internalFormat   = GLES3Impl.GL_RG16F;
    int byte_per_channel = 2;
    if(param.HIGHP_FLOAT){
      internalFormat   = GLES3Impl.GL_RG32F;
      byte_per_channel = 4;
    }
    boolean resized = tex_vel.resize(context, internalFormat, w, h, GLES3Impl.GL_RG, GLES3Impl.GL_FLOAT, GLES3Impl.GL_LINEAR, tex_wrap, 2, byte_per_channel);
    if(resized){
      tex_vel.clear(0);
    }
    return resized;
  ...
  public void blur(int iterations, int radius){
    ...
    tex_tmp.setParamWrap(GLES3Impl.GL_MIRRORED_REPEAT);
    tex_vel.setParamWrap(GLES3Impl.GL_MIRRORED_REPEAT);
  ...
  private void resizeLic(int w, int h){
    tex_lic.resize(context, GLES3Impl.GL_RGBA8, w, h, GLES3Impl.GL_RGBA, GLES3Impl.GL_UNSIGNED_BYTE, GLES3Impl.GL_LINEAR, GLES3Impl.GL_MIRRORED_REPEAT, 4, 1);
//    tex_lic.resize(context, GLES3Impl.GL_RGBA16F, w, h, GLES3Impl.GL_RGBA, GLES3Impl.GL_FLOAT, GLES3Impl.GL_LINEAR, GLES3Impl.GL_MIRRORED_REPEAT, 4, 2);
  }
