...
//    buffer.put(log_len.get(0)-1, (byte) ' ');
  ...
  public static void getProgramValidateStatus( int program_id) {
    ...
    IntBuffer log_len = IntBuffer.allocate(1);
    GLES30.glGetProgramiv(program_id, GLES3Impl.GL_VALIDATE_STATUS, log_len);
  ...
  public int uniformTexture(String uniform_name, int HANDLE_tex, int target){
    ...
    if(loc != -1){
      ...
      GLES30.glActiveTexture(GLES3Impl.GL_TEXTURE0 + untex.loc_idx);
...
//      GLES30.glVertexAttribPointer(LOC_ATTRIB, size, type, normalized, stride, pointer_buffer_offset);
      GLES30.glVertexAttribPointer(LOC_ATTRIB, size, type, normalized, stride, (int) pointer_buffer_offset);
  ...
  public void scissors(int x, int y, int w, int h){
    GLES30.glEnable(GLES3Impl.GL_SCISSOR_TEST);
  ...
  public void drawFullScreenLines(int num_lines, float line_width, boolean smooth){
    if(smooth){
      GLES30.glEnable(GLES3Impl.GL_LINE_SMOOTH);
      GLES30.glHint(GLES3Impl.GL_LINE_SMOOTH_HINT, GLES3Impl.GL_FASTEST);
//      GLES3Impl.glHint(GLES3Impl.GL_LINE_SMOOTH_HINT, GLES3Impl.GL_NICEST);
    } else {
      GLES30.glDisable(GLES3Impl.GL_LINE_SMOOTH);
    }
    GLES30.glLineWidth(line_width);
    GLES30.glDrawArrays(GLES3Impl.GL_LINES, 0, num_lines * 2);
  }
  
  
  public void drawFullScreenPoints(int num_points){
    GLES30.glEnable(GL3.GL_PROGRAM_POINT_SIZE);
    GLES30.glDrawArrays(GLES3Impl.GL_POINTS, 0, num_points);
  }
  
  
//  public void drawFullScreenQuads(int num_quads){
////    GLES3Impl.glDrawArrays(GLES3Impl.GL_QUADS, 0, num_quads * 1);
//    GL2ES3 gl2es3 = GLES3Impl.getGL2ES3();
//    gl2es3.glDrawArraysInstanced(GLES3Impl.GL_TRIANGLE_STRIP, 0, 4, num_quads);  //draw #count quads
//  }
...
//    //
//    {
////       GLES3Impl.glEnable(GLES3Impl.GL_VERTEX_PROGRAM_POINT_SIZE);
//      // GLES3Impl.glEnable(GLES3Impl.GL_POINT_SPRITE); // "gl_pointcoord always zero" ... bug, TODO
//       GLES3Impl.glEnable(GL3.GL_PROGRAM_POINT_SIZE);
//  
//      // GLES3Impl.getGL2().glTexEnvf(GLES3Impl.GL_POINT_SPRITE, GLES3Impl.GL_COORD_REPLACE, GLES3Impl.GL_TRUE);
//  
////      if(smooth){
////        GLES3Impl.glEnable(GLES3Impl.GL_POINT_SMOOTH);
////        GLES3Impl.glHint(GLES3Impl.GL_POINT_SMOOTH_HINT, GLES3Impl.GL_FASTEST);
////      } else {
////        GLES3Impl.glDisable(GLES3Impl.GL_POINT_SMOOTH);
////      }
//    }
//    //
//    // END ... of the problem, TODO
//    
//    DwGLError.debug(GLES3Impl, "DwGLSLProgram.drawFullScreenPoints");
//    GLES3Impl.glDrawArrays(GLES3Impl.GL_POINTS, 0, num_points);
//  }
