...
public class Laplace {
  ...
  public void apply(PGraphicsOpenGL src, PGraphicsOpenGL dst, TYPE kernel) {
    apply(src, dst, kernel, new float[]{0.5f,0.5f});
  }

  public void apply(PGraphicsOpenGL src, PGraphicsOpenGL dst, TYPE kernel, float[] mad) {
    if(src == dst){
  ...
  public void apply(DwGLTexture src, DwGLTexture dst, TYPE kernel) {
    apply(src, dst, kernel, new float[]{1,0});
  }
  
  public void apply(DwGLTexture src, DwGLTexture dst, TYPE kernel, float[] mad) {
    if(src == dst){
      System.out.println("Laplace error: read-write race");
      return;
    }
