...
public class MinMaxGlobal {
  ...
  public void resize(PGraphicsOpenGL pg){
    resize(GLES3Impl.GL_RGBA8, pg.width, pg.height, GLES3Impl.GL_RGBA, GLES3Impl.GL_UNSIGNED_BYTE, 4, 1);
  }
  ...
  public void resize(int iformat, int w, int h, int format, int type, int num_channel, int byte_per_channel){
    ...
    for(int i = 0; i < layers; i++){
      ...
      tex[i].resize(context, iformat, w, h, format, type, GLES3Impl.GL_NEAREST, GLES3Impl.GL_MIRRORED_REPEAT, num_channel, byte_per_channel);
