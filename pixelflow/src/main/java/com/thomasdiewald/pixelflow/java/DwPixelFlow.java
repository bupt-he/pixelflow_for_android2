...
//    if(o != null) key += "["+o.getClass().getCanonicalName()+"]";
  ...
  public void getGLTextureHandle(PGraphicsOpenGL pg, int[] tex_handle){
    ...
    int target     = GLES30.GL_FRAMEBUFFER;
    int attachment = GLES30.GL_COLOR_ATTACHMENT0;
    int[] params   = new int[1];
    GLES30.glBindFramebuffer(target, fbo_handle);
    GLES30.glGetFramebufferAttachmentParameteriv(target, attachment, GLES30.GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE, params, 0);
    if( params[0] == GLES30.GL_TEXTURE){
      GLES30.glGetFramebufferAttachmentParameteriv(target, attachment, GLES30.GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME, params, 0);
  ...
  public void printGL(){
//    GLContext glcontext = GLES30.getContext();

    String opengl_version    = GLES30.glGetString(GLES30.GL_VERSION).trim();
    String opengl_vendor     = GLES30.glGetString(GLES30.GL_VENDOR).trim();
    String opengl_renderer   = GLES30.glGetString(GLES30.GL_RENDERER).trim();
    String opengl_extensions = GLES30.glGetString(GLES30.GL_EXTENSIONS).trim();
    String glsl_version      = GLES30.glGetString(GLES30.GL_SHADING_LANGUAGE_VERSION).trim();
//
...
//    System.out.println();
  ...
  public void printGL_Extensions(){
    String gl_extensions = GLES30.glGetString(GLES30.GL_EXTENSIONS).trim();
...
//    VAL_GL_NVX_gpu_memory_info = GLES30.isExtensionAvailable(GL_NVX_gpu_memory_info);
//    if(VAL_GL_NVX_gpu_memory_info){
//      GLES30.glGetIntegerv(GPU_MEMORY_INFO_DEDICATED_VIDMEM_NVX        , VAL_GPU_MEMORY_INFO_DEDICATED_VIDMEM_NVX        , 0);
//      GLES30.glGetIntegerv(GPU_MEMORY_INFO_TOTAL_AVAILABLE_MEMORY_NVX  , VAL_GPU_MEMORY_INFO_TOTAL_AVAILABLE_MEMORY_NVX  , 0);
//      GLES30.glGetIntegerv(GPU_MEMORY_INFO_CURRENT_AVAILABLE_VIDMEM_NVX, VAL_GPU_MEMORY_INFO_CURRENT_AVAILABLE_VIDMEM_NVX, 0);
//      GLES30.glGetIntegerv(GPU_MEMORY_INFO_EVICTION_COUNT_NVX          , VAL_GPU_MEMORY_INFO_EVICTION_COUNT_NVX          , 0);
//      GLES30.glGetIntegerv(GPU_MEMORY_INFO_EVICTED_MEMORY_NVX          , VAL_GPU_MEMORY_INFO_EVICTED_MEMORY_NVX          , 0);
//    }
//
//    VAL_ATI_meminfo = GLES30.isExtensionAvailable(ATI_meminfo);
//    if(VAL_ATI_meminfo){
//      GLES30.glGetIntegerv(VBO_FREE_MEMORY_ATI         , VAL_VBO_FREE_MEMORY_ATI         , 0);
//      GLES30.glGetIntegerv(TEXTURE_FREE_MEMORY_ATI     , VAL_TEXTURE_FREE_MEMORY_ATI     , 0);
//      GLES30.glGetIntegerv(RENDERBUFFER_FREE_MEMORY_ATI, VAL_RENDERBUFFER_FREE_MEMORY_ATI, 0);
//    }
  }
