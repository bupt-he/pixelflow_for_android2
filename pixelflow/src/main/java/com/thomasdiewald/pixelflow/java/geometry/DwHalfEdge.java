...
public class DwHalfEdge {
  ...
  static public class Mesh{
    ...
    private void create(DwIndexedFaceSetAble ifs){
      ...
      verts = new Vert[verts_count];
      
      // edgemap, for finding edge-pairs
      HashMap<DwPair<Vert>, Edge> edgemap = new HashMap<DwPair<Vert>, Edge>();
      ...
      for(int i = 0, edge_id = 0; i < faces_count; i++){
        ...
        for(int j = 0; j < num_edges; j++){
          edges[edge_id + j] = new Edge(verts[ifs_faces[i][j]]);
        }
        ...
        faces[i] = new Face(edges[edge_id]); // face-link
        edge_id += num_edges;
      }
     ...
     */
    public int getVertexEdges(int vertex_id, Edge[] edges){
    ...
    private void displayPolygons(PGraphics3D pg, Edge edge){
      DwStack<Edge> stack = new DwStack<Edge>();
      stack.push(edge);
    ...
    private void displayQuads(PGraphics3D pg, Edge edge){
      DwStack<Edge> stack = new DwStack<Edge>();
      stack.push(edge);
      float[][] verts = ifs.getVerts();
      float[] v;
    ...
    private void displayTriangles(PGraphics3D pg, Edge edge){
      DwStack<Edge> stack = new DwStack<Edge>();
      stack.push(edge);  
      float[][] verts = ifs.getVerts();
      float[] v;
    ...
    private boolean getFLAG_display(Edge edge){
      return ((edge.FLAG ^ DISPLAY_BIT) & DISPLAY_BIT_MASK) != 0;
    }
    
    private void setFLAG_display(Edge edge){
  ...
  static public int getNumberOfVertexEdges(Vert vert){
    Edge edge = vert.edge;
    Edge iter = edge;
    int count = 0;
    do {
  ...
  static public int getVertexEdges(Vert vert, Edge[] edges){
    Edge edge = vert.edge;
    Edge iter = edge;
    int count = 0;
    do {
